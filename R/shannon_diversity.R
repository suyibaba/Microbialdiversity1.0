...
compare_shannon_two <- function(x, y, shannon_base = exp(1)) {
  if (!requireNamespace("ecolTest", quietly = TRUE)) {
    utils::install.packages("ecolTest")
  }
  result <- ecolTest::Hutcheson_t_test(x, y, shannon.base = shannon_base)
  return(result)
}
...
compare_shannon_multiple <- function(abundance_matrix, shannon_base = exp(1)) {
  ...
  result <- ecolTest::multiple_Hutcheson_t_test(abundance_matrix, 
